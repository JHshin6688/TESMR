...
class PRICAI_ModelX(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        if config['use_center_embedding']:
            self.v_center = torch.tensor(np.load(config['interaction_data_path'] + 'cluster/image_center.npy').astype(np.float32)).to(self.device)
            self.t_center = torch.tensor(np.load(config['interaction_data_path'] + 'cluster/text_center.npy').astype(np.float32)).to(self.device)
    ...
    def full_sort_predict(self, user_list):
        user_all_embeddings, item_all_embeddings, _ = self.forward()
        user_embeddings = user_all_embeddings[user_list]
        scores = torch.matmul(user_embeddings, item_all_embeddings.transpose(0, 1))
        return scores
