...
from FoodRec.utils.dataloader_mine import TrainDataLoader
  ...
  #      valid_score = valid_result['NDCG@20']
   #     return valid_score, valid_result
    ...
    def fit(self, dataset, valid_data=None, test_data=None, hyper_tuple=None, saved=False, verbose=True):
        ...
        train_data = TrainDataLoader(self.config, dataset, use_neg_list=False)
    ...
    def evaluate(self, eval_data, is_test=False, idx=0):
        ...
        # pos_item_dict : {1:[1,2], 4:[4,5,6], ...} user id와 positive item id의 key-value pair
        if is_test == False:
            pos_item_dict = self.model.dataset.valid_user_dict
        else:
            pos_item_dict = self.model.dataset.test_user_dict

        total_user_list = list(pos_item_dict.keys())
        scores = self.model.full_sort_predict(total_user_list)
        key_mapping = {user_id : index for index, user_id in enumerate(pos_item_dict.keys())}

        # training interaction에 있었던 item들 masking
        train_user_dict = self.model.dataset.train_user_dict
        masking = defaultdict(list)
        for eval_user in pos_item_dict.keys():
            if eval_user in train_user_dict:
                items_from_train = train_user_dict[eval_user]
                masking[eval_user] = items_from_train
                        
        for user_id, mask in masking.items():
            scores[key_mapping[user_id], mask] = -1e10
        
        # nusers x topk
        _, topk_index = torch.topk(scores, max(self.config['topk']), dim=-1) 
       
        for _, v in pos_item_dict.items():
            lst_int = [int(x) for x in v]
            pos_items_list.append(lst_int)
            pos_num_list.append(len(lst_int))
            
        return self.evaluator.evaluate(topk_index, (pos_user_list, pos_items_list, pos_num_list), is_test=is_test, idx=idx)

        #for batch_idx, batched_data in tqdm(enumerate(eval_data)):
        #    batch_users = batched_data['u_id']
           
        #    pos_items = []
        #    for i in batch_users:
        #        pos_items.append(pos_item_dict[int(i.item())])
        #        pos_num_list.append(len(pos_item_dict[int(i.item())]))
        ...
        #    pos_user_list.append(batched_data['u_id'])
            
        #return self.evaluator.evaluate(batch_matrix_list, (pos_user_list, pos_items_list, pos_num_list), is_test=is_test, idx=idx)
