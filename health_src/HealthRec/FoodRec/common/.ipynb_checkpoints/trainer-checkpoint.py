...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def evaluate(self, eval_data, is_test=False, idx=0):
        ...
        # pos_item_dict : {1:[1,2], 2:[4,5,6], ...} user id와 positive item id의 key-value pair
        if is_test == False:
        ...
        for batch_idx, batched_data in tqdm(enumerate(eval_data)):
            ...
            #print(f"batched_data u_id shape : {batched_data['u_id'].shape}")
            # {'u_id': tensor([   2,    4,    6,  ..., 9420, 9424, 9431])}
            #print("=====")
            ...
            #print(f"pos_items len : {len(pos_items)}")
            
            # 매 배치마다 4096명 유저의 45630개 item에 대한 점수 tensor를 리턴함
            scores = self.model.full_sort_predict(batched_data)
            # rank and get top-k
            _, topk_index = torch.topk(scores, max(self.config['topk']), dim=-1)  # nusers x topk
            
            #print(f"topk_index shape : {topk_index.shape}")
        ...
        #print(f"batch_matrix_list : {batch_matrix_list}")
        #print("===========================")
