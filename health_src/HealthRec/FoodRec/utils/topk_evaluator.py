...
import pandas as pd
import torch
from FoodRec.common.matrics import metrics_dict
...
class TopKEvaluator(object):
    ...
    def evaluate(self, topk_index, eval_data, is_test=False, idx=0):
        ...
        pos_user, pos_items, pos_len_list = eval_data
        #print(f"pos_user : {pos_user}")
        #print(f"pos_items : {pos_items}")
        #print(f"pos_len_list : {pos_len_list}")
        pos_len = np.asarray(pos_len_list)
        #try:
        #    topk_index = torch.stack(batch_matrix_list).cpu().numpy()
        #except:
        ...
        if self.save_recom_result and is_test:
            ...
            x_df.insert(0, 'id', pos_user)
            x_df.columns = ['id']+['top_'+str(i) for i in range(max_k)]
            x_df = x_df.astype(int)
            x_df.to_csv(file_path, sep='\t', index=False)
        #print("======topk_evaluator======")
        #print("pos_len : ", pos_len, len(pos_len))
        #print("topk_idx : ", topk_index, len(topk_index), topk_index.shape, type(topk_index))
        
        assert len(pos_len) == len(topk_index)
        # if recom right?
        bool_rec_matrix = []
        ...
        #print(f"<bool_rec_mtx> {bool_rec_matrix}, {len(bool_rec_matrix)}, {len(bool_rec_matrix[0])}")
