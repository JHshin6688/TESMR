...
def quick_start(model, dataset, config_dict, save_model=True, mg=False):
    ...
    for hyper_tuple in combinators:
        ...
        # train_data의 inter_matrix함수가 lightGCN에서 사용할 interaction matrix를 리턴
        model = get_model(config['model'])(config, train_data).to(config['device'])
        logger.info(model)

        # trainer loading and initialization
        trainer = get_trainer()(config, model, mg)             # trainer의 optimizer가 이 때 initialize됨
        # debug
        # model training
