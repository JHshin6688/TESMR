...
class TopKEvaluator(object):
    ...
    def evaluate(self, batch_matrix_list, eval_data, is_test=False, idx=0):
        ...
        #print("<pos_len_list>", pos_len_list, len(pos_len_list), type(pos_len_list))
        #print(topk_index, len(topk_index))
        #print(topk_index.shape)
        assert len(pos_len_list) == len(topk_index)
        # if recom right?
        bool_rec_matrix = []
        #print(f"pos_items len : {len(pos_items)}, {type(pos_items)}")
        #print(f"pos_items shape : {pos_items[0], pos_items[1]}")
